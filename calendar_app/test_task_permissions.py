...
class TaskPermissionTestCase(TestCase):
    def setUp(self):
        self.client = Client()
        self.user1 = CustomUser.objects.create_user(username='testuser', password='12345',email="test@email.com")
        self.user2 = CustomUser.objects.create_user(username='testuser2', password='12345',email="test2@email.com")
        self.task = Task.objects.create(name='Test Task 1', description='Test Description', deadlineDay=datetime.date.today(), deadlineTime=datetime.time(12, 0), duration=datetime.timedelta(hours=1), status=False, user=self.user1)
        self.task2 = Task.objects.create(name='Test Task 2', description='Test Description 2', deadlineDay=datetime.date.today(), deadlineTime=datetime.time(12, 0), duration=datetime.timedelta(hours=1), status=False, user=self.user2)
